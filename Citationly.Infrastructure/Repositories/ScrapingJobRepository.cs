...
public class ScrapingJobRepository : IScrapingJobRepository
{
    ...
    public async Task<Guid> CreateJobAsync(ScrapingJob job)
    {
        ...
        var sql = @"
            INSERT INTO ScrapingJobs (OrganizationId, WebsiteId, Url, Status, ScrapeType, TotalPages, ProcessedPages, MaxPages, SuccessfulPages, FailedPages, TotalWords, TotalImages, TotalLinks, StartedAt, CompletedAt, CreatedAt)
            VALUES (@OrganizationId, @WebsiteId, @Url, @Status, @ScrapeType, @TotalPages, @ProcessedPages, @MaxPages, @SuccessfulPages, @FailedPages, @TotalWords, @TotalImages, @TotalLinks, @StartedAt, @CompletedAt, NOW())
            RETURNING Id;";
